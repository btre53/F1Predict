...
const FINDINGS: { tag: string; title: string; body: string }[] = [
  ...
  { tag: "SIGNAL", title: "Backmarkers yield — and modelling it helps", body: "Our position sim used to penalise a fast car stuck behind a backmarker as if the backmarker defended to the death. In reality it yields rather than wreck its tyres in a battle it can't win. Shrinking the held-up penalty with the pace mismatch improved win log-loss (0.178→0.160), top-pick (36%→38%), and — the targeted test — the recovery of fast cars starting at the back (podium log-loss 0.321→0.299). The most positive mechanistic result in the project. See brief 30, which also taxonomises every systematic reason our probability diverges from the market." },
  { tag: "HONEST", title: "Have we exhausted the market gap? Mostly, yes", body: "We audited it directly (brief 29). The model uses prior-race pace + the real quali grid + penalties (all already in the grid) — and that's essentially everything free data offers. The market's residual ~0.005 Brier edge comes from data we structurally lack: fuel-corrected practice pace, setup/upgrade intel, and crowd sentiment. The one free signal we ignore — this-weekend practice — is too noisy to clean up (our FP long-run pace correlates just 0.06 with the quali it should predict, because we can't see fuel loads or engine modes). Post-quali the gap is structural; pre-quali (pole) it's data-limited. We're near the free-data ceiling." },
...
const BRIEFS = [
  ...
  "29 have we exhausted the market gap? (open problems)",
  "30 model-vs-market divergence taxonomy + held-up asymmetry",
];
