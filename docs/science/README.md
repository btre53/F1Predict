...
| [19-car-dna-corner-band.md](19-car-dna-corner-band.md) | Build + validation of brief 16 §2 (task #22), the flagship anti-brand idea. Decompose qualifying telemetry into shape-normalized corner-speed-band factors (low/med/high/straight) × circuit demand. Verdict: KEPT as an Explainer feature only — the decomposition is real + interpretable (McLaren/VER strong in slow corners, Alpine/Sauber on straights; Monza 55% straight, Monaco 0%) but has ZERO incremental predictive lift over scalar pace (corr with quali deviation −0.01 after proper shape-normalization drops scalar-pace corr 0.92→0.18). Served at /cars/dna; NOT in the prediction path. v2: multi-season, traction/braking factors, tie to tyre-deg/lap-time |
